...
var food = []; 
  ...
  function searchImage() {
    ...
    var cardButton = $("<button></button>").text("info")
    cardButton.addClass("btn btn-primary");
    cardButton.attr('data-indexnum', i );
    cardButton.attr('data-food-name', label);
    cardDiv.append(cardButton);
    $("#food-view").append(cardDiv);
   ...
   cardButton.on("click",function(e) {
      ...
      var indexnum = $(this).attr('data-indexnum');
      localStorage.setItem('food',JSON.stringify(food[parseInt(indexnum)]) );
      // console.log(food[parseInt(indexnum)]);
      // localStorage.setItem("food-name", foodName);
      window.location.href='info.html';
     
      // console.log(this);
      // var firstRow = $("<div class= 'col - md-6' >");
      // var infoIndex = parseInt($(this).attr("data-indexnum"));
     
      // // var imageUrl = food[infoIndex].recipe.image;
      // console.log(imageUrl);
      ...
      // // append label of food to image
      // // <h5 class="card-title">Card title</h5>
      // // var label = food[infoIndex].recipe.label;
      // var cardTitle = $("<h5>").text(label);
     
      
      // firstRow.append(cardTitle);
      // $("#first-row").append(firstRow);
