...
function runQuery(numSearch, queryURL) {
  $.ajax({
    ...
    if (parseInt(numTime) === 60) { // 60
      response.hits.forEach(function (element) {
        if (
          element.recipe.totalTime < numTime &&
          element.recipe.totalTime > 30 && // 30
          element.recipe.totalTime != 0 // 0
        ) {
          chosenSearch.push(element.recipe);
        }
    ...
    if (parseInt(numTime) === 120) { // 120
      response.hits.forEach(function (element) {
        if (
          element.recipe.totalTime < numTime &&
          element.recipe.totalTime > 60 && // 60
          element.recipe.totalTime != 0 // 0
        ) {
          chosenSearch.push(element.recipe);
        }
